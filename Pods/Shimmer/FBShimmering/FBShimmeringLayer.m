...
- (void)setShimmeringWithDelay:(CGFloat)shimmeringDelay {
    [self setShimmering:NO];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(shimmeringDelay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self setShimmering:YES];
    });
}
